...
class grade extends control
{
    ...
    {
        $category = $this->loadModel('tree')->getByID($categoryID, 'grade');

        $this->app->loadClass('pager', $static = true);
        $pager = new pager(0, $this->config->grade->recPerPage, $pageID);

        $categoryID = is_numeric($categoryID) ? $categoryID : $category->id;
        $articles   = $this->loadModel('article')->getList('grade', $this->tree->getFamily($categoryID, 'grade'), $orderBy = 'addedDate_desc', $pager);

        $this->view->title      = $this->lang->grade->common;
